...
//This is the function to write the outpt to he correct file and save the image
...
//main function to set up all of the objects that we want for the image
//most important is the spheres
void initScene(std::set<IShape *>&sceneShapes, std::set<Light *>&sceneLights)
{
	//for the planes there are different components the first is the location x,y z then the vector, color and two tone  for theses we want it to be monochromatic so we can see the shadows an stuff better
	...
	Plane *floor = new Plane(Point(0.0f, -2.5f, 0.0f), Vector3D(0.0f, 1.0f, 0.0f),
			Color(0.5f, 0.5f, 0.5f), 0.5f, 0.5f, 0.5f, false);
	//same for the spheres have location x,y,z color rgb, and reflectiveness
	...
	//this will be the main light sources that are used for the tracing
	Light *frontLeftLight = new Light(Point(0.0f, 10.0f, 20.0f),
				Color(1.0f, 1.0f, 1.0f), 0.75f);

	sceneLights.insert(frontLeftLight);

	Light *frontRightLight = new Light(Point(10.0f, 13.0f, 10.0f),
				Color(1.0f, 1.0f, 1.0f), 0.85f);
	
	sceneLights.insert(frontRightLight);

}
//preforms the inersecion of the rays and the objects
